...
public class JiraTicketOrchestrator {
    ...
    private final EventValidator validator; private final IdempotencyService idempotency;
    private final EscalationService escalations; private final ConversationContextService conversations;
    private final JiraIssueBuilder builder; private final JiraClient jira; private final JiraTicketAttemptService attempts;
    private final MetricsService metrics; private final ApplicationProperties properties;
    public JiraTicketOrchestrator(EventValidator validator, IdempotencyService idempotency, EscalationService escalations,
        ...
        this.validator = validator; this.idempotency = idempotency; this.escalations = escalations; this.conversations = conversations;
        this.builder = builder; this.jira = jira; this.attempts = attempts; this.metrics = metrics; this.properties = properties;
    }
    public void process(SupportEscalationRequestedEvent event) {
        validator.validate(event);
        if (idempotency.hasProcessed(event.eventId())) { duplicate(event, "processed_event_exists"); return; }
        if (escalations.claimForProcessing(event.escalationId()).isEmpty()) { duplicate(event, "claim_skipped"); return; }
        ...
        try {
            ...
            attempts.recordRetryableFailure(event, request, e); escalations.markFailed(event.escalationId(), e.getMessage()); metrics.incrementFailed(); throw e;
        } catch (NonRetryableJiraException e) {
            attempts.recordNonRetryableFailure(event, request, e); escalations.markFailed(event.escalationId(), e.getMessage()); metrics.incrementFailed(); throw e;
        }
    }
